...
| 文件格式  | TXT / Markdown / JSON | 章节内容以 .txt 存储，要素数据以 .md 存储，项目配置以 JSON 存储 |
...
> | 存储方式 | 存什么 | 谁在用 |
> |:---------|:-------|:-------|
> | **.txt / .md / JSON 文件** | 章节正文、人物卡片、时间线、实体、项目配置等**正式数据** | 人看的 + AI 读的 |
> | **Orama + IndexedDB** | 要素的 Embedding 向量与检索元数据，用于**语义相似度检索**；Orama 负责检索，IndexedDB 负责本地持久化 | 仅工具内部使用，用户无需关心 |
>
> 章节正文以 `.txt` 保存，要素以 `elements/**/*.md` 保存，JSON 负责项目配置。
> 其中只有 `elements/**/*.md` 会进入 Orama 索引；Orama 索引只是"检索加速用的缓存"。删掉 Orama 索引，工具重新扫描 `elements/` 下的 `.md` 并调用 Embedding API 重建索引即可，数据不会丢失。
...
| **实体** | 武器、坐骑、丹药、信物、武功等可追踪对象 | "青霜剑·裴行舟常用佩剑" |
| **世界观** | 势力、规则、修炼体系、背景设定等扩展要素 | "灵石·蕴含天地灵气" |
...
- **可扩展**：新增要素类型只需新建文件夹
- **可迁移**：整个文件夹拷贝即完成项目迁移
- **可读性**：章节正文使用纯文本（.txt），人物/地点/实体/剧情/时间线/世界观等要素使用 .md，人类可读可编辑
