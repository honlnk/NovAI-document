...
> [!note] 图解
> 每次生成包含两个阶段：**上下文组装**（从四个来源收集信息）和**内容生成**（AI 生成故事并提取要素）。要素的持续积累使得 RAG 检索越来越精准。
...
提供通用的模型配置页面，用户只需填写两项信息即可接入任意兼容 OpenAI 格式的模型：

| 配置项 | 说明 | 示例 |
|:-------|:-----|:-----|
| API 地址 | 模型服务的 Base URL | `https://api.openai.com/v1`、`https://api.deepseek.com/v1` |
| API Key | 认证密钥 | `sk-xxx` |
...
| :-------- | :--------- | :---------- | :---------------- |
| **定位**    | AI 聊天      | 代码编写        | 小说创作              |
| **上下文管理** | 滑动窗口 + 长记忆 | 文件索引 + 工具调用 | **RAG 向量检索**      |
...
### Phase 1 - 核心链路验证
- 通用模型配置页面（API 地址 + Key + 模型名称）
- 项目文件夹结构搭建
- 基础的 RAG 上下文管理（要素提取 → 向量存储 → 相关检索）
- AI 生成内容并写入 .md 文件
- 实时预览
