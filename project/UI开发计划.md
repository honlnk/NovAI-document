...
> [!note] 文档状态
> 本文档记录的是**第一轮正式 UI 开发**（文件树 + 设置页的三栏布局）的计划，各 Phase 已基本完成。
> 主工作区随后启动了 [UI 重构计划](../plans/UI重构计划.md)，将文件树替换为 Activity Bar + 分类面板、设置页改为模态框，并新增编辑模式、选中引用等能力。
> 本文档保留作为第一轮 UI 的历史计划快照，**当前 UI 现状请以 UI 重构计划为准**。
