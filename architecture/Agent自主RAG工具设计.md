...
- ~~当前检索实现仍是过渡方案，正式 Orama 召回、自动增量索引和更好的状态提示仍需补齐。~~（2026-09-20 补丁:Orama 内存向量检索、状态栏 + contentHash 手动增量重建已落地;仍待补的是**自动后台增量重建**与**索引错误降级策略**,见 [当前进度](../project/当前进度.md) 待开始)

下一步建议优先在正式项目里做端到端测试，并根据日志观察结果决定先优化触发提示词、索引状态提示，还是先替换为正式 Orama 召回。(2026-09-20 补丁:Orama 召回已落地,当前瓶颈是真实创作链路验证与 RAG 自动化)
