...
当前 NovAI 已经具备第一版 Agent Loop，并接入了 `ReadFile / EditFile / CreateFile / RenameFile / DeleteFile / ListDirectory / FindFiles` 七个项目文件工具，以及只读语义检索工具 `RagSearch`。但随着产品方向进一步明确，现有工具层仍有几个后续需要补齐的问题：
...
- `RagSearch` 已作为正式 Agent tool 接入新 Agent Loop，但真实创作任务中的触发率、召回质量和后续 `ReadFile(sourcePath)` 使用策略仍需验证。
...
| `ReadFile` | 读取项目内 `.md / .json / .txt` 文件，支持行号、`offset / limit`、大文件完整读取限制，并记录 `readFileState`。章节正文默认是 `.txt`，要素文件与提示词继续用 `.md`。 | 没有读取去重；局部读取后的写入策略仍需继续细化 |
...
| `RagSearch` | 从当前项目的要素索引中召回人物、地点、实体、情节、时间线与世界观上下文，可选 Rerank 后返回给 Agent | 召回质量、触发时机、索引状态提示和完整源文件读取策略仍需持续设计 |

通用 Bash / Shell 不属于 NovAI 当前工具系统范围。具体落地进度以 [当前进度](../project/当前进度.md) 和 [开发日志](../project/开发日志.md) 为准。
