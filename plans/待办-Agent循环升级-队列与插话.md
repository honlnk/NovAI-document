...
**已定稿，待实施。** 源自第 7-8 次提交 review（`323a7b0`/`1ed4ce7`）暴露的痛点：真实创作中频繁被 8 轮上限打断，且运行中发消息被静默吞掉（`ChatPanel.vue:86` 的 `isSending` 拦截）。用户拍板：**循环机制升级为 dsh 的分层架构，好的设计都学过来，UI 跟随修改**。
