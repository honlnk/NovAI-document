...
| 文档 | 状态 | 说明 |
| :--- | :--- | :--- |
| [Element 要素体系优化计划](Element要素体系优化计划.md) | 部分落地，继续推进 | `elements/entities/`、`entity` 类型、实体提取、RAG metadata 和 UI 数量展示已落地；要素模板由《要素模块缺陷补全计划》承接，剧情块拆分、时间线拆分与 Agent 编写要素相关项（行为约束、整理指令）随方案重估暂缓 |
| [要素模块缺陷补全计划](要素模块缺陷补全计划.md) | 进行中 | 要素模块一轮缺陷补全：五类要素模板落地为 core 资产 + 提取 prompt 升级（模板注入、plot 事件粒度、timeline 阶段字段、tags 由模型产出）+ 解析层适配；Agent 主动编写要素的一切（双轨收敛、合并策略、编号、去重、整理指令）经用户拍板整体暂缓 |
