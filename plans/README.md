...
| 文档 | 状态 | 说明 |
| :--- | :--- | :--- |
| [写回面板文件聚合与计数口径修正计划](写回面板文件聚合与计数口径修正计划.md) | 已完成（2026-09-22 结项，`e604360`/`f3d1811`/`1d3891f`，尾巴收尾 `e44e35a`） | 写回面板从操作流水改为按文件聚合（净状态徽章 + 片段 diff 列表 + 展开区限高 30 行），计数从净行数差修正为与 DiffLines 同源的 diffLines 真实增删（修 +0−0、+−2 负值、新建尾部换行多计），面板聚合行数与 GetFileChangeHistory 输出均按片段实时重算、历史账本旧口径数字自愈；created→deleted 呈现删除行而非丢弃。门禁 333 测试全绿 + typecheck + 生产构建；聚合形态与限高滚动已经用户目检确认 |
