...
| 文档 | 状态 | 说明 |
| :--- | :--- | :--- |
| [写回面板文件聚合与计数口径修正计划](写回面板文件聚合与计数口径修正计划.md) | 已完成（2026-09-22 结项，`e604360`/`f3d1811`/`1d3891f`） | 写回面板从操作流水改为按文件聚合（净状态徽章 + 片段 diff 列表 + 展开区限高 30 行），计数从净行数差修正为与 DiffLines 同源的 diffLines 真实增删（修 +0−0、+−2 负值、新建尾部换行多计），聚合行数按片段实时重算使历史账本旧口径数字自愈；created→deleted 呈现删除行而非丢弃。门禁 332 测试全绿 + typecheck + 生产构建；真机目检（聚合形态/限高滚动）待用户复核 |
