...
| 文档 | 状态 | 说明 |
| :--- | :--- | :--- |
| [Agent 控制能力补强计划](Agent控制能力补强计划.md) | 待推进 | 将写入确认、diff 预览、停止运行、工具约束、system prompt 刷新和结构化文件变更事件收束为同一条 Agent 稳定性主线 |
...
| [UI 重构计划](UI重构计划.md) | 部分落地，继续推进 | R1（Activity Bar + 分类面板）、R2（设置模态框）、R3（内容面板三态 + 编辑模式）、R5（选中内容引用）已落地；R4（`@场景` 指令）、R6（`/提取要素` 斜杠指令）、R7（面板拖拽改宽）待实施 |
