...
| 文档 | 状态 | 说明 |
| :--- | :--- | :--- |
| [长会话历史滚动性能计划](长会话历史滚动性能计划.md) | 进行中（2026-09-22 立项即开工） | 借鉴 dsh 分层防线：core 历史分页窗口（首屏尾部 100 条 + 滚动到顶自动 prepend，下标游标无迁移）+ markdown 共享单例与块级渲染（消除流式每 token 整篇重渲）+ 折叠组 until-found 可搜索；不引入虚拟滚动 |
