...
| 文档 | 状态 | 说明 |
| :--- | :--- | :--- |
| [Agent 控制能力补强计划](Agent控制能力补强计划.md) | 部分落地，继续推进 | 将写入确认、diff 预览、停止运行、工具约束、system prompt 刷新和结构化文件变更事件收束为同一条 Agent 稳定性主线。**Step 1 结构化文件变更（`949d540`）、Step 4 停止运行（`8feb3a4`）已完成**，其余 Step（2/3/5/6）待推进 |
